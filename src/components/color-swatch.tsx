        ...
        {colors.map((color, index) => {
          ...
          return (
            <button
              ...
              className="relative h-9 w-9 rounded-full ring-1 ring-(--color-text-muted) transition-transform duration-200 ease-out hover:scale-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--color-accent) focus-visible:ring-offset-2 focus-visible:ring-offset-(--color-background) aria-checked:scale-110 aria-checked:ring-2 aria-checked:ring-(--color-accent) aria-checked:ring-offset-2 aria-checked:ring-offset-(--color-background) disabled:cursor-not-allowed disabled:opacity-80 disabled:hover:scale-100 motion-reduce:transition-none motion-reduce:hover:scale-100 data-[out-of-stock]:after:absolute data-[out-of-stock]:after:inset-0 data-[out-of-stock]:after:rounded-full data-[out-of-stock]:after:bg-[linear-gradient(to_top_right,transparent_calc(50%-1.25px),#1a1a1a_calc(50%-1.25px),#1a1a1a_calc(50%+1.25px),transparent_calc(50%+1.25px))]"
            />
          );
        })}
      </div>
