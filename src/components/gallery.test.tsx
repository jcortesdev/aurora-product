...
describe('<Gallery />', () => {
  it('renders one thumbnail button per image', () => {
    render(<Gallery images={product.colors[0].images} />);
    const toolbar = screen.getByRole('toolbar', { name: /product images/i });
    expect(within(toolbar).getAllByRole('button')).toHaveLength(product.colors[0].images.length);
  });

  it('marks the first thumbnail as active by default', () => {
    render(<Gallery images={product.colors[0].images} />);
    const first = screen.getByRole('button', { name: `View ${product.colors[0].images[0].alt}` });
    expect(first).toHaveAttribute('aria-pressed', 'true');
  });

  it('switches the active image when a thumbnail is clicked', () => {
    render(<Gallery images={product.colors[0].images} />);
    const target = screen.getByRole('button', { name: `View ${product.colors[0].images[2].alt}` });
    fireEvent.click(target);
    expect(target).toHaveAttribute('aria-pressed', 'true');
    const previous = screen.getByRole('button', {
      name: `View ${product.colors[0].images[0].alt}`,
    });
  ...
  it('marks the first stacked image as high fetch priority for LCP', () => {
    render(<Gallery images={product.colors[0].images} />);
    const lead = screen.getByAltText(product.colors[0].images[0].alt);
  ...
  it('lazy-loads non-lead stacked images', () => {
    render(<Gallery images={product.colors[0].images} />);
    const second = screen.getByAltText(product.colors[0].images[1].alt);
  ...
  it('hides inactive pictures from assistive tech', () => {
    const { container } = render(<Gallery images={product.colors[0].images} />);
  ...
  it('moves the active image forward with ArrowRight and back with ArrowLeft', async () => {
    ...
    const second = screen.getByRole('button', { name: `View ${product.colors[0].images[1].alt}` });
  ...
  it('wraps around at the edges', async () => {
    ...
    const last = screen.getByRole('button', {
      name: `View ${product.colors[0].images[product.colors[0].images.length - 1].alt}`,
    });
